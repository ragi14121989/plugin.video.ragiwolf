...
addon = xbmcaddon.Addon('plugin.video.Wolf-Iptv')
...
SOURCES = [{"title": "Wolf-Iptv ", "url": "https://googledrive.com/host/0BzqtJBUy3FRTSTZqZEJkQlI1Qms", "fanart": "https://googledrive.com/host/0BzqtJBUy3FRTRUEyaTROYkktNWs/Wolf3.jpg", "genre": "Tv Live", "date": "10.17.2015", "credits": "Wolf-Iptv", "thumbnail": "https://googledrive.com/host/0BzqtJBUy3FRTRUEyaTROYkktNWs/Wolf6.jpg"}]
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.Wolf-Iptv-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Wolf-Iptv,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Wolf-Iptv,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR cyan][B]- FAVORITES CHANNEL Wolf-Iptv [/COLOR][/B][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(Wolf-Iptv,Urlresolver donot support this domain. - ,5000)")
    return resolver
...
def download_file(name, url):
        ...
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Wolf-Iptv', 'Do you want to add this file as a source?')
        if ret:
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Wolf-Iptv Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
